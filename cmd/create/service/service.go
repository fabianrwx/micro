...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err := protoGenService(serviceName); err != nil {
...
func protoGenService(service string) error {
	// run go generate
	cmd := exec.Command("task", "proto")
	cmd.Dir = service
	if err := cmd.Run(); err != nil {
...
func updateTaskfile(tf *core.TaskFile, service string) (*core.TaskFile, error) {
	...
	if task, exists := tf.Tasks["proto"]; exists {
		...
		newCmd := fmt.Sprintf("protoc --proto_path=%s/proto --go_out=%s/pb --go_opt=paths=source_relative --go-grpc_out=%s/pb --go-grpc_opt=paths=source_relative --grpc-gateway_out=%s/pb --grpc-gateway_opt=paths=source_relative --openapiv2_out=docs/swagger --openapiv2_opt=allow_merge=true,merge_file_name=%s %s/proto/*.proto", service, service, service, service, service, service)
